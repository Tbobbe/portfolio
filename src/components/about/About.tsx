            ...
            <div className=" w-screen pt-[20rem] mb-[20rem] " id={`${data.title}`}>
                <div ref={slice} className={` bg-[#3D405B] w-full md:h-[50rem] h-[40rem] pt-[3rem] skew-y-6 transition-all duration-1000 ease-linear ${sliceInView ? 'translate-y-10 opacity-100' : 'opacity-0'}`}>
                    <div className=" bg-[#E07A5F] w-full md:h-[44rem] h-[34rem]">
                        <div ref={profileImg} className={` relative -skew-y-6 md:h-[30rem] md:w-[30rem] h-[20rem] w-[20rem] 2xl:mx-0 mx-auto 2xl:left-[15%] 2xl:top-[10rem] top-[-15rem] md:border-8 border-4 border-[#F4F1DE] rounded-full transition-all duration-1000 ease-linear ${profileImgInView ? '-translate-y-20 opacity-100' : 'opacity-0'}`}>
